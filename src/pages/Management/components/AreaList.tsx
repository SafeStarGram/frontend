      ...
      <div className="grid grid-cols-2 gap-4">
        {areas.map((area) => (
          <div key={area.id} className="bg-white rounded-2xl shadow-sm overflow-hidden p-6">
            <h4 className="text-lg font-bold text-gray-900 mb-6 text-center">{area.areaName}</h4>
            <div className="mb-6">
              <img 
                src={area.imageUrl || "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=300&h=200&fit=crop"} 
                alt={area.areaName}
            ...
            {area.subAreas && area.subAreas.length > 0 && (
              <div className="mb-4">
                <p className="text-sm text-gray-600 mb-2">서브 구역: {area.subAreas.length}개</p>
                <div className="flex flex-wrap gap-1">
                  ...
                  {area.subAreas.length > 3 && (
                    <span className="text-xs text-gray-500">+{area.subAreas.length - 3}개 더</span>
                  )}
                </div>
              </div>
            )}
            <button 
              onClick={() => navigate("/areadetail", { state: { areaId: area.id } })}
